...
# Purpose: Examine abundance/densities of Chinook winter presmolts and explore preferred or target habitat conditions
...
fh_win_champ_2017 = loadRData("../QRFcapacity/data/fh_win_champ_2017.rda")
...
chnk_win_qrtl = quantile(chnk_win_n$log_fish_dens, probs = c(0, 0.25, 0.50, 0.75, 1))

# plot log transformed fish densities with quartiles
chnk_win_p = chnk_win_n %>%
  ggplot() +
  geom_histogram(aes(x = log_fish_dens, fill = Watershed), bins = 50) +
  geom_vline(xintercept = chnk_win_qrtl, color = 'blue') +
  theme_bw() +
...
# reduce chnk_win_df in preparation for evaluation
chnk_win_df = chnk_win_df %>%
...
# next, let's parse the chnk_win_df by Watershed, Tier1
# by watershed
wtr = unique(chnk_win_df$Watershed)
for(w in wtr) {
  tmp = filter(chnk_win_df, Watershed == as.character(w)) %>%
    mutate(qrtl = cut_number(log_fish_dens, n = 4, labels = c('Q1','Q2','Q3','Q4'))) %>%
    mutate(qrtl = recode(qrtl,
                         `Q1` = 'Rest',
                         `Q2` = 'Rest',
                         `Q3` = 'Rest'))
  assign(paste('chnk_win_', make_clean_names(w), sep = ''), tmp)
}

# by tier 1 category
tr1 = unique(chnk_win_df$Tier1)
for(t in tr1) {
  tmp = filter(chnk_win_df, Tier1 == as.character(t)) %>%
    mutate(qrtl = cut_number(log_fish_dens, n = 4, labels = c('Q1','Q2','Q3','Q4'))) %>%
    mutate(qrtl = recode(qrtl,
                         `Q1` = 'Rest',
                         `Q2` = 'Rest',
                         `Q3` = 'Rest'))
  assign(paste('chnk_win_', make_clean_names(t), sep = ''), tmp)
}

# list of data frames
df_list = list(Entiat = chnk_win_entiat, 
               Lemhi = chnk_win_lemhi,
               Methow = chnk_win_methow,
               SFS = chnk_win_south_fork_salmon,
               UGR = chnk_win_upper_grande_ronde, 
               Wenatchee = chnk_win_wenatchee, 
               Yankee = chnk_win_yankee_fork, # watersheds
               pool = chnk_win_pool, 
               riffle = chnk_win_riffle,
               run = chnk_win_run,
               ssc = chnk_win_ssc) # Tier 1
...
fh_plot(chnk_win_lemhi, substrate)
...
#     pl = plot_list[[p]]
#     tmp_p = fh_plot(df, pl)
#     ggsave(paste0('figures/chnk_win_fh_plots/', names(df_list)[d], '_', names(plot_list)[p], '.png'))
...
sig_tst_results
ggsave('figures/chnk_win_significant_comps.png')
...
p_val_p = tst_results %>%
  ...
  facet_wrap(~ key)
p_val_p
ggsave('figures/chnk_win_p_values.png')
